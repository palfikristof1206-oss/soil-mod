// =====================================================================
// 🌱 SOIL-MOD v5 – COMPLETE ECOSYSTEM SIMULATION
// Egy fájlba integrált teljes verzió – mixinggel kiegészítve
// =====================================================================

console.log("🌱 SOIL-MOD v5 LOADING...");
...
// Búza növekedési ciklus
addTick("wheat_seed", function (p) {
    p.g = p.g || 0;
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (!below) return;

    // Talaj minősége befolyásolja a növekedést
    ...
    // Mycorrhiza boost
    if (neighbors(p).find(x => x.element === "mycorrhiza")) {
        p.g += 0.02;
    }

    if (p.g > 1) {
        changePixel(p, "wheat_sprout");
    }
});
...
addTick("tomato_root", function (p) {
    p.g = p.g || 0;
    p.g += 0.03;
    if (p.g > 1.2) changePixel(p, "tomato_stem");
});

addTick("tomato_stem", function (p) {
    p.g = p.g || 0;
    p.g += 0.02;
    if (p.g > 1.8) changePixel(p, "tomato_leaf");
});

addTick("tomato_leaf", function (p) {
    p.g = p.g || 0;
    p.g += 0.03;
    if (p.g > 2.2) changePixel(p, "tomato_flower");
});

addTick("tomato_flower", function (p) {
    p.g = p.g || 0;
    p.g += 0.04;
    if (p.g > 2.8) changePixel(p, "tomato_fruit");
});
...
addTick("potato_root", function (p) {
    p.g = p.g || 0;
    p.g += 0.03;
    if (p.g > 1.3) changePixel(p, "potato_plant");
});

addTick("potato_plant", function (p) {
    p.g = p.g || 0;
    p.g += 0.02;
    if (p.g > 2) changePixel(p, "potato_tuber");
});
...
addTick("tree_sapling", function (p) {
    ...
    if (p.g > 5) {
        ...
        // Levelek növesztése körben
        neighbors8(p).forEach(n => {
            if (n.element === "empty" || n.element === "air") {
                changePixel(n, "tree_leaves");
            }
        });
    }
});

// Fa levelek idővel lehullanak
...
addTick("earthworm", function (p) {
    ...
    } else if (t.element === "fertile_soil") {
        changePixel(t, "rich_soil");
    } else if (t.element === "compost" && chance(30)) {
        changePixel(t, "humus");
    }
    ...
    if (chance(0.1) && neighbors(p).find(x => x.element === "earthworm")) {
        ...
        if (emptySpot) {
            changePixel(emptySpot, "earthworm");
        }
    }
});

// worm_tunnel visszaalakul
addTick("worm_tunnel", function (p) {
    if (!cd(p, "tunnel_fill", 30000)) return;
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (below && below.element?.includes("soil")) {
        changePixel(p, below.element);
    } else {
        changePixel(p, "fertile_soil");
    }
});
...
addTick("mycorrhiza", function (p) {
    if (!cd(p, "myco_spread", 3000)) return;

    // Terjedés talajban
    let soilNeighbors = neighbors(p).filter(n =>
        ["dirt", "fertile_soil", "rich_soil", "super_fertile_soil",
         "forest_soil", "compost", "humus", "sandy_soil", "loam"].includes(n.element)
    );

    if (soilNeighbors.length > 0 && chance(15)) {
        let target = soilNeighbors[randInt(0, soilNeighbors.length - 1)];
        if (target.element !== "mycorrhiza") {
    ...
    // Talajjavítás
    neighbors(p).forEach(n => {
        if (n.element === "rich_soil" && chance(5)) {
            changePixel(n, "super_fertile_soil");
        }
        if (n.element === "compost" && chance(8)) {
            changePixel(n, "humus");
        }
    });

    // Elhalás
    if (soilNeighbors.length === 0 && chance(2)) {
        changePixel(p, "humus");
    }
});
...
addTick("irradiated_soil", function (p) {
    ...
    if (chance(0.01)) {
        changePixel(p, "depleted_soil");
    }
});

// living_soil kialakulása
addTick("rich_soil", function (p) {
    if (neighbors(p).find(x => x.element === "earthworm") && chance(0.5)) {
        changePixel(p, "living_soil");
    }
});

// old_growth_soil
addTick("forest_soil", function (p) {
    if (neighbors(p).find(x => x.element === "humus") && chance(0.3)) {
        changePixel(p, "old_growth_soil");
    }
});

// fertile_volcanic_soil
...
// Szerves anyagok égési beállításai
...
// Talaj pH változása műtrágya hatására
addTick("acidic_soil", function (p) {
    if (neighbors(p).find(x => x.element === "lime") && chance(10)) {
        changePixel(p, "neutral_soil");
    }
});

addTick("alkaline_soil", function (p) {
    if (neighbors(p).find(x => x.element === "fertilizer_n") && chance(5)) {
        changePixel(p, "neutral_soil");
    }
});

// Műtrágya túladagolás → savasodás
addTick("super_fertile_soil", function (p) {
    if (!cd(p, "ph_check", 10000)) return;
    let fertCount = neighbors(p).filter(x => x.element?.includes("fertilizer")).length;
    if (fertCount >= 2 && chance(fertCount * 2)) {
        changePixel(p, "acidic_soil");
    }
});

// Növények pH preferenciája
addTick("wheat_seed", function (p) {
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (below && below.element === "acidic_soil") {
        p.g = (p.g || 0) * 0.5; // lassabb növekedés savas talajban
    }
});

addTick("tomato_seed", function (p) {
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (below && below.element === "alkaline_soil") {
        p.g = (p.g || 0) * 0.6;
    }
});
...
addTick("rain_cloud", function (p) {
    ...
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (below && (below.element === "empty" || below.element === "air")) {
        changePixel(below, "water");
    }
    // Talaj nedvesítése
    if (below && below.element?.includes("soil") && chance(30)) {
        changePixel(below, "moist_soil");
    }
    // Felhő lassan elfogy
    if (chance(0.1)) {
        changePixel(p, "steam");
...
addTick("storm_cloud", function (p) {
    ...
    // Villám esélye
    if (chance(0.5) && below) {
        changePixel(below, "fire");
    }
    if (chance(0.05)) {
        changePixel(p, "rain_cloud");
    }
});

// Savas eső → savasítja a talajt
addTick("acid_rain_cloud", function (p) {
    if (!cd(p, "acid_rain", 500)) return;
    let below = pixelMap?.[p.x]?.[p.y + 1];
    if (below && (below.element === "empty" || below.element === "air")) {
        changePixel(below, "acid");
    }
...
// Évszakok alapvető hatása (egyszerűsített)
// Ősz: levelek hullanak gyakrabban
// Tél: hó esik, növények lassabban nőnek
// Tavasz: gyorsabb növekedés
// Nyár: gyorsabb száradás

// Egyszerűsített évszak rendszer (globális változóval)
window._soilSeason = window._soilSeason || "spring";
window._soilSeasonTimer = window._soilSeasonTimer || 0;

// Évszak váltás (minden elemre ható globális tick)
...
["dirt", "fertile_soil", "rich_soil", "super_fertile_soil", "sandy_soil"].forEach(name => {
    addTick(name, function (p) {
        ...
        if (waterNeighbor && chance(0.3)) {
            changePixel(p, "eroded_soil");
        }
    });
});

// eroded_soil → depleted_soil (tovább romlik)
...
// Compaction – nehéz elemek hatása
...
addTick("tiller", function (p) {
    ...
    neighbors8(p).forEach(n => {
        ...
        } else if (n.element === "compacted_soil") {
            changePixel(n, "fertile_soil");
        } else if (n.element === "cracked_soil") {
            changePixel(n, "fertile_soil");
        }
    });
});
...
addTick("greenhouse", function (p) {
    ...
    // Környező növények gyorsabban nőnek
    neighbors(p).forEach(n => {
        ...
        // Talaj melegebb
        if (n.element?.includes("soil") && n.temp < 30) {
            n.temp += 0.5;
        }
    });
});
...
// Biome magok – elhelyezve automatikusan alakítják a környezetet
...
addTick("forest_biome_seed", function (p) {
    if (!cd(p, "forest_biome", 5000)) return;
    neighbors8(p).forEach(n => {
        if (n.element === "dirt" && chance(15)) changePixel(n, "forest_soil");
        if (n.element === "empty" && chance(0.5)) changePixel(n, "tree_seed");
        if (n.element === "empty" && chance(2)) changePixel(n, "leaf");
    });
    // Terjedés
    if (chance(1)) {
        let n = neighbors(p)[randInt(0, 3)];
        if (n && n.element === "empty") changePixel(n, "forest_biome_seed");
    }
});

// Swamp biome
addTick("swamp_biome_seed", function (p) {
    if (!cd(p, "swamp_biome", 5000)) return;
    neighbors8(p).forEach(n => {
        if (n.element === "dirt" && chance(10)) changePixel(n, "peaty_soil");
        if (n.element === "empty" && chance(3)) changePixel(n, "water");
        if (n.element === "water" && chance(5)) changePixel(n, "mud");
    });
    if (chance(1)) {
        let n = neighbors(p)[randInt(0, 3)];
        if (n && n.element === "empty") changePixel(n, "swamp_biome_seed");
    }
});

// Plains biome
addTick("plains_biome_seed", function (p) {
    if (!cd(p, "plains_biome", 5000)) return;
    neighbors8(p).forEach(n => {
        if (n.element === "dirt" && chance(10)) changePixel(n, "loam");
        if (n.element === "empty" && chance(0.5)) changePixel(n, "wheat_seed");
    });
    if (chance(1)) {
        let n = neighbors(p)[randInt(0, 3)];
        if (n && n.element === "empty") changePixel(n, "plains_biome_seed");
    }
});

// Desert biome
addTick("desert_biome_seed", function (p) {
    if (!cd(p, "desert_biome", 5000)) return;
    neighbors8(p).forEach(n => {
        if (n.element === "dirt" && chance(15)) changePixel(n, "sand");
        if (n.element === "water" && chance(20)) changePixel(n, "steam");
        if (n.element?.includes("soil") && !["sand", "sandy_soil", "rocky_soil"].includes(n.element) && chance(5)) {
            changePixel(n, "sandy_soil");
        }
    });
    if (chance(1)) {
        let n = neighbors(p)[randInt(0, 3)];
        if (n && n.element === "empty") changePixel(n, "desert_biome_seed");
    }
});

// Volcanic biome
addTick("volcanic_biome_seed", function (p) {
    if (!cd(p, "volcanic_biome", 5000)) return;
    neighbors8(p).forEach(n => {
        if (n.element === "dirt" && chance(10)) changePixel(n, "volcanic_soil");
        if (n.element === "empty" && chance(0.2)) changePixel(n, "lava");
        if (n.element === "stone" && chance(5)) changePixel(n, "magma");
    });
    if (chance(0.5)) {
        let n = neighbors(p)[randInt(0, 3)];
        if (n && n.element === "empty") changePixel(n, "volcanic_biome_seed");
    }
});

console.log("✅ [13/14] Biomes loaded");
...
addReaction("irrigation_system", "water", { elem1: "irrigation_system" }); // önfenntartó

// =============================
// ÉVSZAK FRISSÍTÉS (globális)
// =============================
// Egy meglévő gyakori elemre akasztjuk az évszak frissítést
...
// =====================================================================
// 🎉 SOIL-MOD v5 TELJESEN BETÖLTVE
// =====================================================================
console.log("══════════════════════════════════════");
console.log("🌍 SOIL-MOD v5 FULLY LOADED");
console.log("══════════════════════════════════════");
