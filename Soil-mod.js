// =====================================================================
// 🌱 SOIL-MOD v5.2 – STABILIZED RENDER (No SwapPixel, Safe Ticks)
// =====================================================================

console.log("🌱 SOIL-MOD v5.2 LOADING...");

// =====================================================================
// 1. CORE HELPERS (STABILIZÁLT)
// =====================================================================
...
// BIZTONSÁGOS PIXEL CSERE – ellenőrzi a célpontot
function changePixelSafe(p, newElement) {
    if (!p || !newElement || !elements[newElement]) return false;
    if (!pixelMap || !pixelMap[p.x] || !pixelMap[p.x][p.y]) return false;
    if (pixelMap[p.x][p.y].element === newElement) return false; // már az
...
// SWAP PIXEL TILTVA – renderelési bugot okoz!
// function swapPixel(p1, p2) { ... } // NEM HASZNÁLJUK!
...
console.log("✅ [1/14] Core Helpers loaded (SwapPixel ELTÁVOLÍTVA)");
...
addTick("humus", function (p) {
    ...
    neighbors(p).forEach(n => {
        if (["dirt", "fertile_soil", "rich_soil"].includes(n.element) && chance(3)) changePixel(n, "forest_soil");
    });
});
addTick("super_fertile_soil", function (p) {
    if (!cd(p, "nutrient_drain", 30000)) return;
    let hasNutrient = neighbors(p).find(x => ["humus", "compost", "manure", "fertilizer_n", "fertilizer_p", "fertilizer_k"].includes(x.element));
...
addTick("tree_sapling", function (p) {
    p.g = p.g || 0; p.g += 0.01;
    if (p.g > 5) { changePixel(p, "tree_trunk"); neighbors8(p).forEach(n => { if (n.element === "empty" || n.element === "air") changePixel(n, "tree_leaves"); }); }
});
addTick("tree_leaves", function (p) { if (!cd(p, "leaf_fall", 20000)) return; if (chance(0.5)) changePixel(p, "leaf"); });

console.log("✅ [5/14] Plants loaded");

// =====================================================================
// 6. BIOLÓGIA (EARTHWORM + MIKORRHIZA STABILIZÁLT)
// =====================================================================
...
addTick("earthworm", function (p) {
    ...
    let movable = ["empty", "dirt", "fertile_soil", "rich_soil", "compost", "humus", "worm_tunnel", "sandy_soil", "loam", "forest_soil", "depleted_soil"];
    if (movable.includes(t.element)) {
        changePixel(p, "worm_tunnel");
        changePixel(t, "earthworm");
    }
...
// ---- MIKORRHIZA (STABILIZÁLT – NINCS SWAP) ----
...
addTick("mycorrhiza", function (p) {
    ...
    // 1. NÖVEKEDÉS ÜRES HELYRE (biztonságos)
    let emptyNeighbors = neighbors(p).filter(n => n.element === "empty" || n.element === "air");
    if (emptyNeighbors.length > 0 && chance(30)) {
        let target = emptyNeighbors[randInt(0, emptyNeighbors.length - 1)];
        changePixel(target, "mycorrhiza"); // csak simán nő
    }

    // 2. TERJEDÉS TALAJBA (régi helyén mycorrhiza marad, talajt NEM tolja arrébb)
    let soilNeighbors = neighbors(p).filter(n => allSoils.includes(n.element) && n.element !== "mycorrhiza");
    if (soilNeighbors.length > 0 && chance(15)) {
        let target = soilNeighbors[randInt(0, soilNeighbors.length - 1)];
        changePixel(target, "mycorrhiza"); // benövi a talajt, a régi mycorrhiza is marad
    }

    // 3. NÖVÉNYEK GYORSÍTÁSA
    neighbors(p).forEach(n => {
...
console.log("✅ [6/14] Biology loaded (Mycorrhiza: STABIL – nincs swap)");

// =====================================================================
// 7-14. RÖVIDÍTVE A STABILITÁS MIATT (minden korábbi funkció megvan)
// =====================================================================
...
addTick("irradiated_soil", function (p) {
    neighbors(p).forEach(n => { if (["wheat_seed", "wheat_sprout", "tomato_seed", "potato_seed", "plant"].includes(n.element) && chance(5)) changePixel(n, "mutant_plant"); });
    if (chance(0.01)) changePixel(p, "depleted_soil");
});
addTick("rich_soil", function (p) { if (neighbors(p).find(x => x.element === "earthworm") && chance(0.5)) changePixel(p, "living_soil"); });
addTick("forest_soil", function (p) { if (neighbors(p).find(x => x.element === "humus") && chance(0.3)) changePixel(p, "old_growth_soil"); });
addReaction("volcanic_soil", "water", { elem1: "fertile_volcanic_soil" });
console.log("✅ [7/14] Mutations loaded");

// 8. HŐMÉRSÉKLET
["humus", "compost", "manure", "leaf_litter", "peat"].forEach(name => { if (elements[name]) { elements[name].tempHigh = 200; elements[name].stateHigh = "fire"; elements[name].burn = 80; } });
["dirt", "sandy_soil", "fertile_soil", "rich_soil", "super_fertile_soil", "forest_soil", "loam", "clay_soil"].forEach(name => { if (elements[name]) { elements[name].tempHigh = 1200; elements[name].stateHigh = "molten_silicate"; } });
...
["super_fertile_soil", "fertile_soil", "rich_soil", "forest_soil", "humus"].forEach(name => { addTick(name, function (p) { if (p.temp > 500 && chance(10)) changePixel(p, "burnt_soil"); }); });
console.log("✅ [8/14] Temperature System loaded");

// 9. pH
addTick("acidic_soil", function (p) { if (neighbors(p).find(x => x.element === "lime") && chance(10)) changePixel(p, "neutral_soil"); });
addTick("alkaline_soil", function (p) { if (neighbors(p).find(x => x.element === "fertilizer_n") && chance(5)) changePixel(p, "neutral_soil"); });
addTick("super_fertile_soil", function (p) { if (!cd(p, "ph_check", 10000)) return; let fertCount = neighbors(p).filter(x => x.element?.includes("fertilizer")).length; if (fertCount >= 2 && chance(fertCount * 2)) changePixel(p, "acidic_soil"); });
...
addTick("rain_cloud", function (p) { if (!cd(p, "rain", 500)) return; let below = pixelMap?.[p.x]?.[p.y + 1]; if (below && (below.element === "empty" || below.element === "air")) changePixel(below, "water"); if (below && below.element?.includes("soil") && chance(30)) changePixel(below, "moist_soil"); if (chance(0.1)) changePixel(p, "steam"); });
addTick("storm_cloud", function (p) { if (!cd(p, "storm", 300)) return; let below = pixelMap?.[p.x]?.[p.y + 1]; if (below && (below.element === "empty" || below.element === "air")) changePixel(below, "water"); if (chance(0.5) && below) changePixel(below, "fire"); if (chance(0.05)) changePixel(p, "rain_cloud"); });
addTick("acid_rain_cloud", function (p) { if (!cd(p, "acid_rain", 500)) return; let below = pixelMap?.[p.x]?.[p.y + 1]; if (below && (below.element === "empty" || below.element === "air")) changePixel(below, "acid"); if (below && ["fertile_soil", "rich_soil", "super_fertile_soil", "forest_soil"].includes(below.element) && chance(20)) changePixel(below, "acidic_soil"); if (chance(0.05)) changePixel(p, "rain_cloud"); });
addTick("snow_cloud", function (p) { if (!cd(p, "snow", 500)) return; let below = pixelMap?.[p.x]?.[p.y + 1]; if (below && (below.element === "empty" || below.element === "air")) changePixel(below, "snow"); if (chance(0.05)) changePixel(p, "steam"); });
addTick("moist_soil", function (p) { if (!cd(p, "dry_out", 15000)) return; let above = pixelMap?.[p.x]?.[p.y - 1]; if (!above || above.element !== "water") { if (chance(10)) { let below = pixelMap?.[p.x]?.[p.y + 1]; if (below && below.element?.includes("soil")) changePixel(p, below.element); else changePixel(p, "dirt"); } } });
window._soilSeason = window._soilSeason || "spring"; window._soilSeasonTimer = window._soilSeasonTimer || 0;
function updateSeason() { window._soilSeasonTimer++; if (window._soilSeasonTimer > 50000) { window._soilSeasonTimer = 0; let seasons = ["spring", "summer", "autumn", "winter"]; let idx = seasons.indexOf(window._soilSeason); window._soilSeason = seasons[(idx + 1) % 4]; } }
...
["dirt", "fertile_soil", "rich_soil", "super_fertile_soil", "sandy_soil"].forEach(name => { addTick(name, function (p) { let waterNeighbor = neighbors(p).find(x => x.element === "water"); if (waterNeighbor && chance(0.3)) changePixel(p, "eroded_soil"); }); });
addTick("eroded_soil", function (p) { if (!cd(p, "erode_further", 30000)) return; if (chance(2)) changePixel(p, "depleted_soil"); });
["stone", "iron", "steel", "concrete"].forEach(heavy => { if (elements[heavy]) { addReaction("fertile_soil", heavy, { elem1: "compacted_soil" }); addReaction("rich_soil", heavy, { elem1: "compacted_soil" }); addReaction("super_fertile_soil", heavy, { elem1: "compacted_soil" }); } });
addTick("compacted_soil", function (p) { if (!cd(p, "crack", 40000)) return; if (chance(1)) changePixel(p, "cracked_soil"); });
["dirt", "fertile_soil", "rich_soil", "clay_soil"].forEach(name => { addTick(name, function (p) { let waterCount = neighbors(p).filter(x => x.element === "water").length; if (waterCount >= 3 && chance(waterCount * 2)) changePixel(p, "waterlogged_soil"); }); });
...
addTick("composter", function (p) { if (!cd(p, "compost_work", 3000)) return; neighbors(p).forEach(n => { if (["plant", "wood", "leaf", "grass", "dead_plant", "wheat_sprout", "wheat_stem", "wheat_head", "tomato_leaf", "tomato_stem", "potato_plant", "tree_leaves", "leaf_litter", "tree_sapling"].includes(n.element)) changePixel(n, "compost"); }); });
addTick("irrigation_system", function (p) { if (!cd(p, "irrigate", 2000)) return; neighbors(p).forEach(n => { if (n.element?.includes("soil") && n.element !== "waterlogged_soil" && n.element !== "moist_soil") changePixel(n, "moist_soil"); if (["wheat_seed", "wheat_sprout", "tomato_seed", "tomato_root", "potato_seed", "potato_root", "tree_seed", "tree_sapling"].includes(n.element)) { if (n.g !== undefined) n.g += 0.05; } }); });
addTick("tiller", function (p) { if (!cd(p, "till", 3000)) return; neighbors8(p).forEach(n => { if (n.element === "dirt") changePixel(n, "fertile_soil"); else if (n.element === "compacted_soil") changePixel(n, "fertile_soil"); else if (n.element === "cracked_soil") changePixel(n, "fertile_soil"); }); });
addTick("greenhouse", function (p) { if (!cd(p, "greenhouse_effect", 5000)) return; neighbors(p).forEach(n => { if (n.element?.includes("seed") || n.element?.includes("sprout") || n.element?.includes("root") || n.element?.includes("stem") || n.element?.includes("sapling")) { if (n.g !== undefined) n.g += 0.1; } if (n.element?.includes("soil") && n.temp < 30) n.temp += 0.5; }); });
console.log("✅ [12/14] Machines loaded");

// =====================================================================
// 13. BIOME-OK (STABILIZÁLT – LASSABB, BIZTONSÁGOS)
// =====================================================================
...
addTick("forest_biome_seed", function (p) {
    ...
    p._biomeAge = p._biomeAge || 0; p._biomeAge++;
    if (!cd(p, "forest_main", 4000)) return; // 4mp cooldown
    let isActive = p._biomeAge > 200;
    let gC = isActive ? 15 : 5;
    neighbors8(p).forEach(n => {
        if (!n || !pixelMap?.[n.x]?.[n.y]) return;
        if (["dirt", "sand"].includes(n.element) && chance(gC)) changePixel(n, "forest_soil");
        if (n.element === "empty" && isActive && chance(gC * 0.3)) changePixel(n, "tree_seed");
    });
    if (isActive && chance(2)) { let nn = neighbors(p).filter(n => n && ["dirt", "sand"].includes(n.element)); if (nn.length > 0) changePixel(nn[0], "forest_biome_seed"); }
});
// Swamp
addTick("swamp_biome_seed", function (p) {
    ...
    p._biomeAge = p._biomeAge || 0; p._biomeAge++;
    if (!cd(p, "swamp_main", 4000)) return;
    let isActive = p._biomeAge > 200;
    let gC = isActive ? 12 : 4;
    neighbors8(p).forEach(n => {
        if (!n) return;
        if (["dirt", "sand"].includes(n.element) && chance(gC)) changePixel(n, "peaty_soil");
        if (n.element === "empty" && isActive && chance(gC * 0.3)) changePixel(n, "water");
    });
    if (isActive && chance(2)) { let nn = neighbors(p).filter(n => n && ["dirt", "sand"].includes(n.element)); if (nn.length > 0) changePixel(nn[0], "swamp_biome_seed"); }
});
// Plains
addTick("plains_biome_seed", function (p) {
    ...
    p._biomeAge = p._biomeAge || 0; p._biomeAge++;
    if (!cd(p, "plains_main", 4000)) return;
    let isActive = p._biomeAge > 200;
    let gC = isActive ? 15 : 5;
    neighbors8(p).forEach(n => {
        if (!n) return;
        if (["dirt", "sand"].includes(n.element) && chance(gC)) changePixel(n, "loam");
        if (n.element === "empty" && isActive && chance(gC * 0.3)) changePixel(n, "wheat_seed");
    });
    if (isActive && chance(2)) { let nn = neighbors(p).filter(n => n && ["dirt", "sand"].includes(n.element)); if (nn.length > 0) changePixel(nn[0], "plains_biome_seed"); }
});
// Desert
addTick("desert_biome_seed", function (p) {
    ...
    p._biomeAge = p._biomeAge || 0; p._biomeAge++;
    if (!cd(p, "desert_main", 4000)) return;
    let isActive = p._biomeAge > 200;
    let gC = isActive ? 18 : 6;
    neighbors8(p).forEach(n => {
        if (!n) return;
        if (["dirt", "loam", "fertile_soil"].includes(n.element) && chance(gC)) changePixel(n, "sand");
        if (n.element === "water" && chance(gC)) changePixel(n, "steam");
    });
    if (isActive && chance(2)) { let nn = neighbors(p).filter(n => n && ["dirt", "sand"].includes(n.element)); if (nn.length > 0) changePixel(nn[0], "desert_biome_seed"); }
});
// Volcanic
addTick("volcanic_biome_seed", function (p) {
    ...
    p._biomeAge = p._biomeAge || 0; p._biomeAge++;
    if (!cd(p, "volcanic_main", 4000)) return;
    let isActive = p._biomeAge > 200;
    let gC = isActive ? 10 : 3;
    neighbors8(p).forEach(n => {
        if (!n) return;
        if (["dirt", "sand"].includes(n.element) && chance(gC)) changePixel(n, "volcanic_soil");
        if (n.element === "empty" && isActive && chance(gC * 0.1)) changePixel(n, "ash");
    });
    if (isActive && chance(1)) { let nn = neighbors(p).filter(n => n && ["dirt", "stone"].includes(n.element)); if (nn.length > 0) changePixel(nn[0], "volcanic_biome_seed"); }
});

console.log("✅ [13/14] Biomes loaded (STABILIZÁLT)");

// =====================================================================
// 14. INTEGRÁCIÓ + MIXING
// =====================================================================
...
addTick("dirt", function (p) { if (!cd(p, "season_update", 10000)) return; updateSeason(); });

console.log("✅ [14/14] Integration + Mixing loaded");

console.log("══════════════════════════════════════");
console.log("🌍 SOIL-MOD v5.2 STABILIZED – READY");
console.log("══════════════════════════════════════");